...
Part/cour markers are also parsed as structured match clues. `Part 1`, `Part 2`, `Cour 2`, `Zenpen`, `Kouhen`, `前編`, and `後編` are compared against provider titles, original titles, and aliases. A mismatched part/cour marker rejects the candidate; a matching marker can override season numbers derived only from local filenames, which covers cour folders whose files continue numbering as `S01E12+`. Explicit folder season markers such as `Season 2` still have to agree with provider season metadata.
...
Root-folder metadata matching cross-checks detected local episode count against provider episode count whenever the provider or cache has an episode total. A cached/provider match is rejected when the local file count is greater than the matched anime's expected episodes. Season matching is conservative: local title or file-path hints such as `Season 2` or `S02E01` must agree with the provider season, and provider season 2+ results are not auto-accepted when the local root has no explicit season hint. During import and database normalization, Nyaarr also rejects two entries with the same resolved display title unless their provider identity, expected episode count, and detected local episode count all match. Conflicting same-title imports keep their local folder name and are flagged for manual metadata verification.
