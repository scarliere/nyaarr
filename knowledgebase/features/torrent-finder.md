...
The torrent finder starts when a user clicks Add on an Add Anime metadata result.
...
5. Subtract confidence when a prioritized subber list exists but the candidate release group is not in it, or when the candidate has no reported seeders or no detected group.
...
Prioritized subbers are stored as `settings.preferred_subbers`, a user-defined ordered list of release groups/subbers. The current scorer treats membership as a confidence boost; ordering is reserved for a later stronger weighting pass.
...
Ignored torrents are skipped during future candidate selection so a rejected compromised torrent is not retried for the same or later search pass.
