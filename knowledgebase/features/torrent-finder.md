...
Configured preferred subbers also keep alias searching active when the primary
title only finds other groups. Results that match the title but cannot be parsed
as an episode or batch no longer stop alias fallback. For example, a noisy Kill
Blue result cannot prevent the finder from continuing to Kill Ao and selecting
a prioritized SubsPlease release. The first usable non-prioritized result is
retained as fallback if no title variant finds a preferred group.
