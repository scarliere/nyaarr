...
- For anime with an active queue and an existing `local_path`, the fast queue job rescans that folder before evaluating status. Newly arrived files therefore update episode rows and card progress immediately instead of waiting for broader storage maintenance.
